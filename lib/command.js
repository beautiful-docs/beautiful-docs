...
(function() {
  var S, _, arg, argv, bfdocs, destDir, fs, generateIndex, generateManifest, handleManifest, i, len, loadManifests, lock, manifests, options, parts, path, ref, startServer, switches, v;
  ...
  generateManifest = function(manifest, callback = null) {
    var dest, opts;
    dest = destDir;
  ...
  generateIndex = function(filename, callback = null) {
  ...
  loadManifests = function() {
    var filename, j, len1, results;
    results = [];
    for (j = 0, len1 = argv.length; j < len1; j++) {
      filename = argv[j];
      if (fs.statSync(filename).isDirectory()) {
        console.log(`Generating documentation from directory '${filename}'`);
        results.push(bfdocs.createManifestFromDir(filename, handleManifest));
      } else {
        console.log(`Generating documentation from manifest '${filename}'`);
        results.push(bfdocs.open(filename, handleManifest));
      }
    }
    return results;
  };
